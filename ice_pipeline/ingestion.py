...
class ICEIngestionManager:
    ...
    def __init__(self):
        ...
        self.ice_script_path = os.getenv(
            "ICE_SCRIPT_PATH", "/app/ice_enhanced_ingestion.py"
        )
    ...
    async def run_ingestion(self) -> IngestionResult:
        ...
        try:
            ...
            if process.returncode == 0:
                logger.info(
                    f"ICE ingestion completed successfully in {execution_time:.2f}s"
                )
                result = IngestionResult(
                    success=True,
                ...
                logger.error(
                    f"ICE ingestion failed with return code {process.returncode}"
                )
                result = IngestionResult(
                    success=False,
                    status=IngestionStatus.ERROR,
...
__all__ = ["ICEIngestionManager", "IngestionStatus", "IngestionResult", "ice_manager"]
