...
try:
    from .ingestion import ICEIngestionManager, IngestionStatus, IngestionResult
...
class ExcelConversionRequest(BaseModel):
    file_path: str = Field(..., min_length=1, max_length=1000, description="Path to the Excel file")
    output_format: str = Field(default="csv", description="Output format (csv, json, parquet)")
    include_metadata: bool = Field(default=False, description="Whether to include file metadata")
...
class ExcelConversionResponse(BaseModel):
    ...
    output_file: str = Field(..., min_length=1, description="Path to the converted output file")
    rows_processed: int = Field(..., ge=0, description="Number of rows processed")
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="File metadata if requested")
...
async def health_check():
    ...
    return {
        ...
        "version": "1.0.0",
        "services": {
            "ice_ingestion": "active" if ingestion_manager else "unavailable"
        }
    }
...
async def trigger_ice_ingestion(background_tasks: BackgroundTasks):
    ...
    if not ingestion_manager:
        raise HTTPException(status_code=503, detail="ICE ingestion manager not available")
    ...
    return ICEIngestionResponse(
        success=True,
        status="triggered",
        message="ICE ingestion started in background"
    )
...
# Helper functions
def process_excel_conversion(file_path: str, output_format: str, include_metadata: bool) -> Dict[str, Any]:
    """
