...
class TestICEIngestionPerformance:
    ...
    def test_multiple_environment_validations(self, ingestion_manager):
        ...
        with patch('ice_pipeline.ingestion.Path.exists', return_value=True):
            ...
            assert end_time - start_time < 2.0  # Less than 2 seconds for 1000 validations
...
class TestICEStressTests:
    ...
    async def test_ingestion_stress_test(self, ingestion_manager):
        ...
        with patch.object(ingestion_manager, 'run_ingestion', side_effect=mock_successful_ingestion):
    ...
    async def test_error_handling_stress(self, ingestion_manager):
        ...
        with patch.object(ingestion_manager, 'run_ingestion', side_effect=mock_intermittent_failure):
            with patch.object(ingestion_manager, 'validate_environment', return_value=True):
...
class TestICEScalabilityTests:
    ...
    async def test_async_scalability(self):
        ...
        for batch_size in batch_sizes:
            ...
            assert processing_time < sequential_time_estimate * 0.5  # At least 2x speedup
