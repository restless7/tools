...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            "/home/sebastiangarcia/ice-data-staging/reports/ingestion_v3.log"
        ),
...
class EnhancedStagingIngestionManager:
    ...
    def extract_excel_csv_data(self) -> Dict[str, Any]:
        ...
        extractor = ExcelDataExtractor(
            source_dir=str(self.source_dir), output_dir=str(self.extracted_csvs_dir)
        )
        ...
        self.stats["reference_files_stored"] = processing_result["stats"][
    ...
    def scan_directory_structure(self) -> Dict[str, Any]:
        ...
        self.stats["students_from_directories"] = results["stats"][
            "person_records_created"
        ]

        logger.info("=" * 80)
        logger.info("DIRECTORY SCAN SUMMARY")
        logger.info(
            f"Students from Directories: {self.stats['students_from_directories']}"
        )
    ...
    def merge_csv_and_directory_data(
        self, csv_data: Dict[str, Any], directory_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def insert_reference_data_to_database(
        self, reference_files: List[Dict[str, Any]], run_id: int
    ):
        """
        ...
        with self.conn.cursor() as cur:
            for ref_file in reference_files:
                try:
                    ...
                    logger.warning(
                        f"Error inserting reference file {ref_file['sheet_name']}: {e}"
                    )
                    continue
    ...
    def normalize_and_stage_documents(
        self, students: List[Dict[str, Any]], documents: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for doc in documents:
            try:
                ...
                doc_count = sum(
                    1 for d in normalized_docs if d["student_id"] == student_id
                )
                ...
                logger.error(
                    f"Error normalizing document {doc.get('file_name', 'unknown')}: {e}"
                )
                continue
    ...
    def run_full_ingestion(self):
        ...
        try:
            ...
            normalized_documents = self.normalize_and_stage_documents(
                merged_data["students"], merged_data["documents"]
            )
            ...
            base_manager = StagingIngestionManager(
                str(self.source_dir), str(self.staging_dir), self.db_url
            )
            base_manager.conn = self.conn
            base_manager.insert_staging_data(
                merged_data["students"], normalized_documents
            )
            ...
            self.insert_reference_data_to_database(
                merged_data["reference_files"], run_id
            )
    ...
    def generate_final_report(
        self, merged_data: Dict[str, Any], run_id: int, start_time: datetime
    ):
...
"""
        ...
        for program, count in sorted(
            program_counts.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        report_file = (
            self.reports_dir
            / f"ingestion_v3_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
