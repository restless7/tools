...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/sebastiangarcia/ice-data-staging/reports/ingestion.log'),
...
class StagingIngestionManager:
    ...
    def improve_document_classification(self, file_path: str, original_type: str) -> str:
        """
        ...
        type_keywords = {
            ...
            'TRANSCRIPT': ['transcript', 'notas', 'grades', 'academic record', 'certificado de notas', 'student status', 'proof of student'],
            'PHOTO': ['photo', 'foto', 'picture', 'imagen', '2x2', 'passport photo'],
            'FINANCIAL': ['bank', 'banco', 'financial', 'balance', 'solvencia', 'sponsorship', 'fee', 'refund', 'disclosure'],
            'MEDICAL': ['medical', 'medico', 'health', 'salud', 'vaccination', 'vacuna'],
            'ID_CARD': ['cedula', 'id card', 'identification', 'dni', 'id ', 'identif', 'ssn', 'social security'],
            'LETTER': ['letter', 'carta', 'recommendation', 'recomendacion', 'job offer', 'offer letter'],
            'RESUME': ['resume', 'cv', 'hoja de vida', 'curriculum', 'resume '],
            'CONTRACT': ['contract', 'contrato', 'agreement', 'acuerdo', 'cu digital', 'cu-', 'cu20'],
        ...
        for doc_type, keywords in type_keywords.items():
            if any(keyword in filename_lower for keyword in keywords):
                logger.debug(f"Improved classification: {original_type} → {doc_type} for {Path(file_path).name}")
                return doc_type
        
        # If still OTHER, keep it but log
        if original_type == 'OTHER':
            logger.debug(f"Could not improve classification for: {Path(file_path).name}")
    ...
    def generate_report(
        ...
        # Save to file
        report_file = self.reports_dir / f'ingestion_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    ...
    def run_full_ingestion(self):
        ...
        try:
            ...
            for doc in results['documents']:
                ...
                student = next(
                    (s for s in results['student_records'] if s['student_id'] == doc['student_id']),
                    None
                )
                ...
                student_doc_count = sum(1 for d in normalized_documents if d['student_id'] == doc['student_id'])
                
                # Copy and normalize
                normalized_doc = self.copy_and_normalize_document(student, doc, student_doc_count)
            ...
            logger.info(f"✔ Created {len(student_folders_created)} student folders with metadata")
