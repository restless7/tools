...
class StagingToProduction:
    ...
    def __init__(
        self, staging_db_url: str, production_db_url: str, dry_run: bool = False
    ):
        self.staging_conn = psycopg2.connect(
            staging_db_url, cursor_factory=RealDictCursor
        )
        self.prod_conn = psycopg2.connect(
            production_db_url, cursor_factory=RealDictCursor
        )
    ...
    def find_or_create_person(self, staging_data: dict) -> Tuple[Optional[str], bool]:
        ...
        with self.prod_conn.cursor() as cur:
            ...
            if staging_data.get("email"):
                cur.execute(
                    "SELECT id FROM persons WHERE email = %s", (staging_data["email"],)
                )
                result = cur.fetchone()
                if result:
            ...
            if staging_data.get("birth_date"):
                ...
                if result:
                    logger.debug(
                        f"Found person by name+birthdate: {staging_data['full_name']}"
                    )
                    self._update_person(result["id"], staging_data)
                    return (result["id"], False)
            ...
            if staging_data.get("phone"):
                ...
                if result:
                    logger.debug(
                        f"Found person by name+phone: {staging_data['full_name']}"
                    )
                    self._update_person(result["id"], staging_data)
                    return (result["id"], False)
            ...
            if (
                not staging_data.get("email")
                and not staging_data.get("phone")
                and not staging_data.get("cedula")
            ):
                ...
                if result:
                    logger.debug(
                        f"Found person by fuzzy name match: {search_name} → {result['full_name']}"
                    )
                    self._update_person(result["id"], staging_data)
                    return (result["id"], False)
        ...
        with self.prod_conn.cursor() as cur:
            ...
            if not email:
                if data.get("phone"):
                    ...
                    normalized = re.sub(r"[^a-z0-9]", "", data["full_name"].lower())[
                        :50
                    ]
    ...
    def migrate_students(self):
            ...
            for student in staging_students:
                try:
                    ...
                    person_data = {
                        ...
                        "original_name": student.get(
                            "full_name"
                        ),  # Use full_name as original
                    }
                    person_id, created = self.find_or_create_person(person_data)

                    if not person_id:
                        logger.warning(
                            f"Could not find/create person for student: {student['full_name']}"
                        )
                    ...
                    if self.dry_run:
                        ...
                        logger.info(
                            f"[DRY RUN] Would create student for person: {student['full_name']}"
                        )
                    else:
                        with self.prod_conn.cursor() as prod_cur:
                            ...
                            if existing_student:
                                ...
                                logger.debug(
                                    f"Updated student for person: {student['full_name']}"
                                )
                            else:
                                # Create new student
                                prod_cur.execute(
                                ...
                                self.stats["students_created"] += 1
                                logger.info(
                                    f"Created student for person: {student['full_name']}"
                                )
        ...
        logger.info(
            f"Students: {self.stats['students_created']} created, {self.stats['students_updated']} updated, {self.stats['students_skipped']} skipped"
        )
    ...
    def migrate_leads(self):
        ...
        with self.staging_conn.cursor() as staging_cur:
            ...
            for lead in staging_leads:
                try:
                    ...
                    if not person_id:
                        logger.warning(
                            f"Could not find/create person for lead: {lead['full_name']}"
                        )
                    ...
                    if self.dry_run:
                        ...
                        logger.info(
                            f"[DRY RUN] Would create lead for person: {lead['full_name']}"
                        )
                    else:
                        with self.prod_conn.cursor() as prod_cur:
                            prod_cur.execute(
                            ...
                            if existing_lead:
                                ...
                                logger.debug(
                                    f"Updated lead for person: {lead['full_name']}"
                                )
                            else:
                                # Create new lead
                                prod_cur.execute(
                                ...
                                self.stats["leads_created"] += 1
                                logger.info(
                                    f"Created lead for person: {lead['full_name']}"
                                )
        ...
        logger.info(
            f"Leads: {self.stats['leads_created']} created, {self.stats['leads_updated']} updated, {self.stats['leads_skipped']} skipped"
        )
    ...
    def migrate_documents(self):
            ...
            for doc in staging_documents:
                try:
                    ...
                    with self.prod_conn.cursor() as prod_cur:
                        ...
                        if not student_id:
                            logger.warning(
                                f"Could not find student for document: {doc['original_file_name']} (person: {doc['full_name']})"
                            )
                        ...
                        if existing_doc:
                            logger.debug(
                                f"Document already exists (checksum match): {doc['original_file_name']}"
                            )
                            self.stats["documents_skipped"] += 1
                        else:
                            if self.dry_run:
                                ...
                                logger.info(
                                    f"[DRY RUN] Would create document: {doc['original_file_name']}"
                                )
                            else:
                                ...
                                self.stats["documents_created"] += 1
                                logger.info(
                                    f"Created document: {doc['original_file_name']} for student {student_id}"
                                )

                except Exception as e:
                    logger.error(
                        f"Error migrating document {doc.get('original_file_name', 'unknown')}: {e}"
                    )
                    if not self.dry_run:
                        self.prod_conn.rollback()
                    self.stats["documents_skipped"] += 1
        ...
        logger.info(
            f"Documents: {self.stats['documents_created']} created, {self.stats['documents_skipped']} skipped"
        )
    ...
    def run_full_migration(self):
        ...
        logger.info("=" * 80)
        logger.info(
            f"🚀 V3 STAGING → PRODUCTION MIGRATION {'(DRY RUN)' if self.dry_run else ''}"
        )
        logger.info("=" * 80)
        ...
        try:
            ...
            logger.info(
                f"Persons: {self.stats['persons_created']} created, {self.stats['persons_updated']} updated"
            )
            logger.info(
                f"Students: {self.stats['students_created']} created, {self.stats['students_updated']} updated, {self.stats['students_skipped']} skipped"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate V3 staging data to production ICE bot database"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=os.getenv(
            ...
            "postgresql://postgres:224207bB@localhost:5432/leads_project",
        ),
    )
    ...
    logger.info(
        f"Staging DB: {args.staging_db.split('@')[1] if '@' in args.staging_db else args.staging_db}"
    )
    logger.info(
        f"Production DB: {args.production_db.split('@')[1] if '@' in args.production_db else args.production_db}"
    )
    logger.info(f"Mode: {'DRY RUN' if args.dry_run else 'LIVE MIGRATION'}")

    migrator = StagingToProduction(
        args.staging_db, args.production_db, dry_run=args.dry_run
    )
