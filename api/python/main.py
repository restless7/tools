...
async def root():
    return {
        ...
        "version": "1.0.0",
        "endpoints": ["/excel/convert", "/excel/sheets", "/ice/ingest", "/health"],
    }
...
async def ingest_ice_database(
    background_tasks: BackgroundTasks, request: ICEIngestionRequest
):
    """
    ...
    try:
        ...
        logger.info(
            f"Starting ICE ingestion with force_reprocess={request.force_reprocess}"
        )
        ...
        if ingestion_result["success"]:
            ...
            raise HTTPException(
                status_code=500, detail=f"Ingestion failed: {error_msg}"
            )
...
async def get_staging_student_details(student_id: str):
    ...
    try:
        ...
        return {"student": dict(student), "documents": [dict(d) for d in documents]}
...
async def get_staging_leads(limit: int = 50, offset: int = 0):
    """
...
async def get_reference_data(limit: int = 50, offset: int = 0):
    """
...
async def trigger_v3_ingestion(
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            raise HTTPException(
                status_code=500, detail=f"V3 Ingestion failed: {result.stderr[-1000:]}"
            )
    ...
    try:
        ...
        if result["success"]:
            ...
            return {
                ...
                "students_deleted": (
                    result["stats"].get("students_deleted", 0) if execute else 0
                ),
                "documents_removed": (
                    result["stats"].get("documents_orphaned", 0) if execute else 0
                ),
            ...
            raise HTTPException(
                status_code=500, detail=result.get("error", "Sanitization failed")
            )
...
async def delete_student_manually(student_id: str):
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(
                status_code=404, detail=result.get("error", "Student not found")
            )
...
async def cleanup_staging_data(confirm: bool = False):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Some cleanup operations failed"
            )
