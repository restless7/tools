...
class LocalIngestionLoader:
    ...
    def _scan_data_files(
        self, directory: Path, file_types: List[str] = None
    ) -> List[Path]:
        """
        ...
        try:
            ...
            for file_path in student_files:
                ...
                if df is not None:
                    ...
                    logger.info(
                        f"Loaded {len(records)} student records from {file_path.name}"
                    )

            self.stats.student_records = len(student_records)
            logger.info(
                f"✔ Loaded {len(student_records)} total student records from {self.stats.student_files_processed} files"
            )
        ...
        try:
            ...
            for file_path in lead_files:
                ...
                if df is not None:
                    ...
                    logger.info(
                        f"Loaded {len(records)} lead records from {file_path.name}"
                    )

            self.stats.lead_records = len(lead_records)
            logger.info(
                f"✔ Loaded {len(lead_records)} total lead records from {self.stats.lead_files_processed} files"
            )
                ...
                for file_path in student_dir.iterdir():
                    ...
                    try:
                        ...
                        metadata = self.storage_adapter.get_metadata(
                            str(file_path), student_id=student_id
                        )
    ...
    def load_all_data(self) -> Dict[str, Any]:
        ...
        logger.info("=" * 80)
        logger.info(
            f"✔ Found {self.stats.student_records} student records from {self.stats.student_files_processed} files"
        )
        logger.info(
            f"✔ Found {self.stats.lead_records} lead records from {self.stats.lead_files_processed} files"
        )
