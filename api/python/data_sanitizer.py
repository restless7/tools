...
class DataSanitizer:
    ...
    BLACKLIST = {
        ...
        "LALIFE ADVENTURES",
        "L A LIFE ADVENTURES",
        "1. LA LIFE ADVENTURES",
        "1.LA LIFE ADVENTURES",
        "AAG ALLIANCE ABROAD GROUP",
        "ALLIANCE ABROAD GROUP",
        "2.AAG ALLIANCE ABROAD GROUP",
        "2. AAG ALLIANCE ABROAD GROUP",
        "IEE INTERNATIONAL EDUCATIONAL EXCHANGE",
        "INTERNATIONAL EDUCATIONAL EXCHANGE",
        "3. IEE INTERNATIONAL EDUCATIONAL EXCHANGE",
        "3.IEE INTERNATIONAL EDUCATIONAL EXCHANGE",
        ...
        "PREGUNTAS WORK AND TRAVEL 2024",
        "VIDEOS DE PANTALLA",
        "VIDEOSDE PANTALLA",
        "OFICINA",
        "TODOS",
    ...
    def delete_false_positives(
        self, false_positives: List[Dict[str, Any]], dry_run: bool = False
    ):
        """
        ...
        logger.info(
            f"{'[DRY RUN] ' if dry_run else ''}Deleting {len(false_positives)} false positive records..."
        )
        ...
        with self.conn.cursor() as cur:
            for fp in false_positives:
                ...
                logger.info(
                    f"{'[DRY RUN] ' if dry_run else ''}Deleting: {full_name} (student_id={student_id}, {doc_count} docs)"
                )

                if not dry_run:
                    # Delete documents first (foreign key constraint)
                    cur.execute(
                        "DELETE FROM staging_document WHERE student_id = %s",
                        (student_id,),
                    )
                    ...
                    # Delete student
                    cur.execute(
                        "DELETE FROM staging_student WHERE id = %s", (student_id,)
                    )
                    self.stats["students_deleted"] += cur.rowcount

                    # Delete person
                    cur.execute(
                        "DELETE FROM staging_person WHERE id = %s", (person_id,)
                    )
    ...
    def delete_student_by_id(self, student_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.conn.cursor() as cur:
                ...
                # Delete documents
                cur.execute(
                    "DELETE FROM staging_document WHERE student_id = %s", (student_id,)
                )
                docs_deleted = cur.rowcount
                ...
                # Delete person
                cur.execute(
                    "DELETE FROM staging_person WHERE id = %s", (student["person_id"],)
                )

                self.conn.commit()

                logger.info(
                    f"Manually deleted: {student['full_name']} ({docs_deleted} docs)"
                )
    ...
    def sanitize(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if false_positives:
                ...
                for fp in false_positives:
                    logger.info(
                        f"  - {fp['full_name']:<40} ({fp['program']:<20} | {fp['document_count']} docs)"
                    )
            ...
            logger.info(
                f"False Positives Found:  {self.stats['false_positives_found']}"
            )
            ...
            if not dry_run:
                ...
                logger.info(
                    f"Documents Removed:      {self.stats['documents_orphaned']}"
                )
            ...
            return {"success": False, "error": str(e), "stats": self.stats.copy()}
