...
def check_dependencies():
    ...
    deps = {"pandas": HAS_PANDAS, "openpyxl": HAS_OPENPYXL, "ice_pipeline": False}
    ...
    try:
        ...
        deps["ice_pipeline"] = (
            ice_db_path.exists()
            and (ice_db_path / "enhanced_ingest_drive_wat.py").exists()
        )
    except:
        pass
...
async def health_check():
    ...
    return HealthStatus(
        ...
        services={
            "excel_converter": (
                "active" if (deps["pandas"] and deps["openpyxl"]) else "maintenance"
            ),
    ...
    try:
        ...
        for sheet_name in sheet_names:
            try:
                ...
                logger.info(
                    f"Created: {csv_filename} ({len(df)} rows, {len(df.columns)} columns)"
                )
...
async def trigger_ice_ingestion(
    background_tasks: BackgroundTasks, force_reprocess: bool = False
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail="ICE pipeline components not available"
        )
    except Exception as e:
