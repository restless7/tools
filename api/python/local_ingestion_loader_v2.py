...
class LocalIngestionLoaderV2:
    ...
    COLUMN_MAPPINGS = {
        'full_name': ['NOMBRE COMPLETO', 'Nombre Completo', 'Name', 'Full Name', 'full_name', 'fullname'],
        'email': ['CORREO', 'Correo', 'Email', 'E-mail', 'email', 'correo'],
        'phone': ['CELULAR', 'Celular', 'Teléfono', 'Telefono', 'Phone', 'phone', 'celular'],
        'address': ['DIRECCION', 'Dirección', 'Direccion', 'Address', 'address', 'direccion'],
        'id_number': ['CEDULA', 'Cédula', 'Cedula', 'ID', 'id_number', 'cedula'],
        'birth_date': ['FECHA DE NACIMIENTO', 'Fecha de Nacimiento', 'Birth Date', 'birth_date'],
    ...
    def __init__(self, base_dir: str):
        ...
        logger.info(f"LocalIngestionLoaderV2 initialized with base_dir: {self.base_dir}")
    ...
    def _extract_field_value(self, row: Dict, df: pd.DataFrame, field_name: str) -> Optional[str]:
        """
        ...
        try:
            ...
            for item in base_path.rglob('*'):
                if item.is_dir():
                    ...
                    has_docs = any(
                        f.is_file() for f in item.iterdir()
                    )
    ...
    def match_documents_to_students(
        self, 
        student_dirs: List[Tuple[str, Path]]
        ...
        """
        logger.info(f"Matching documents from {len(student_dirs)} directories to students...")
        ...
        for student_name, doc_dir in student_dirs:
            ...
            if not student_id:
                logger.warning(f"Could not match directory '{student_name}' to any student")
        ...
        if unmatched_count > 0:
            logger.warning(f"⚠ {unmatched_count} directories could not be matched to students")
    ...
    def _infer_document_type(self, file_name: str) -> str:
        ...
        if any(keyword in file_name_lower for keyword in ['passport', 'pasaporte']):
            ...
            return 'VISA'
        elif any(keyword in file_name_lower for keyword in ['diploma', 'certificate', 'certificado']):
            return 'DIPLOMA'
        elif any(keyword in file_name_lower for keyword in ['resume', 'cv', 'hoja de vida']):
            return 'OTHER'
    ...
    def create_students_from_directories(
        ...
        """
        logger.info(f"Creating Person and Student records from {len(student_dirs)} directories...")
        ...
        for student_name, doc_dir in student_dirs:
            ...
            logger.debug(f"Created record from directory: {student_name} (student_id: {student_id})")
        
        self.stats.person_records_created = len(all_records)
        self.stats.student_records_created = len(all_records)
        
        logger.info(f"✔ Created {len(all_records)} Person/Student records from directories")
        ...
        for csv_file in csv_files:
            try:
                ...
                for idx, row in df.iterrows():
                    ...
                    matching_record['email'] = self._extract_field_value(row_dict, df, 'email')
                    matching_record['phone'] = self._extract_field_value(row_dict, df, 'phone')
                    matching_record['address'] = self._extract_field_value(row_dict, df, 'address')
                    matching_record['id_number'] = self._extract_field_value(row_dict, df, 'id_number')
                    
                    # Parse birth date
                    birth_date_str = self._extract_field_value(row_dict, df, 'birth_date')
                    if birth_date_str:
                        try:
                            matching_record['birth_date'] = pd.to_datetime(birth_date_str).date()
                        except:
    ...
    def run_ingestion(
        ...
        try:
            ...
            student_records = self.create_students_from_directories(document_dirs, program)
            ...
            logger.info(f"Student Records Created: {self.stats.student_records_created}")
            logger.info(f"Records Enriched from CSV: {self.stats.records_enriched_from_csv}")
...
if __name__ == '__main__':
    ...
    csv_files = [Path("/home/sebastiangarcia/Downloads/data_ingestion/drive-download-20251105T055300Z-1-001/Au Pair/AU PAIR.xlsx")]
