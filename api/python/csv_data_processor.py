...
class PersonDataProcessor:
    ...
    ENROLLED_KEYWORDS = ['inscrito', 'inscrita', 'enrolled', 'active', 'activo', 'contrato']
    INTERESTED_KEYWORDS = ['interesada', 'interesado', 'interested', 'prospecto', 'lead']
    ...
    def extract_persons_from_csv(
        ...
        for idx, row in df.iterrows():
            try:
                ...
                email = str(row[email_col]).strip().lower() if email_col and pd.notna(row[email_col]) else None
                ...
                cedula = str(row[cedula_col]).strip() if cedula_col and pd.notna(row[cedula_col]) else None
                birth_date = row[birth_col] if birth_col and pd.notna(row[birth_col]) else None
    ...
    def _determine_status_from_sheet(self, sheet_name: str, classification: str) -> str:
        ...
        if any(kw in sheet_lower for kw in ['inscrita', 'inscrito', 'registered', 'enrolled', 'active']):
            return 'ENROLLED'
        
        # Check for interested/lead
        if any(kw in sheet_lower for kw in ['interesada', 'interesado', 'interested', 'lead', 'prospecto']):
            return 'INTERESTED'
