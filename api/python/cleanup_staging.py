...
class StagingCleaner:
    ...
    def clean_database(self, confirm: bool = False):
        ...
        """
        if not confirm:
            logger.warning("⚠️  Database cleanup requires confirmation. Set confirm=True")
            return False
    ...
    def clean_documents(self, confirm: bool = False):
        ...
        """
        if not confirm:
            logger.warning("⚠️  Document cleanup requires confirmation. Set confirm=True")
            return False
        ...
        try:
            ...
            total_files = sum(1 for folder in student_folders if folder.is_dir() 
                            for _ in folder.iterdir())
            
            logger.info(f"Found {len(student_folders)} student folders with {total_files} files")
    ...
    def clean_reports(self, confirm: bool = False):
        ...
        try:
            # Count before deletion
            report_files = [f for f in reports_dir.iterdir() if f.is_file() and f.suffix in ['.txt', '.json']]
    ...
    def get_staging_stats(self):
        ...
        if reports_dir.exists():
            stats['files']['reports'] = len([f for f in reports_dir.iterdir() if f.is_file()])
