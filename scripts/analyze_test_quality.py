...
class TestQualityAnalyzer:
    ...
    def analyze_test_execution_quality(self, artifacts_dir: Path) -> Dict[str, Any]:
        ...
        for xml_file in junit_files:
            if 'junit' in xml_file.name.lower() or 'test' in xml_file.name.lower():
                ...
                if suite_data:
                    ...
                    analysis['failed_tests'] += suite_data.get('failures', 0) + suite_data.get('errors', 0)
                    ...
                    for test_case in suite_data.get('test_cases', []):
                        ...
                        test_results[test_key].append(test_case.get('status', 'unknown'))
        
        analysis['passed_tests'] = analysis['total_tests'] - analysis['failed_tests'] - analysis['skipped_tests']
        
        # Calculate rates and averages
        if analysis['total_tests'] > 0:
            analysis['success_rate'] = (analysis['passed_tests'] / analysis['total_tests']) * 100
            analysis['failure_rate'] = (analysis['failed_tests'] / analysis['total_tests']) * 100
            analysis['average_execution_time'] = total_execution_time / len(analysis['test_suites']) if analysis['test_suites'] else 0.0
        ...
        analysis['flaky_test_percentage'] = (analysis['flaky_tests'] / analysis['total_tests'] * 100) if analysis['total_tests'] > 0 else 0.0
        analysis['problematic_tests'] = sorted(flaky_tests, key=lambda x: x['failure_rate'], reverse=True)[:10]
    ...
    def analyze_coverage_quality(self, artifacts_dir: Path) -> Dict[str, Any]:
        ...
        for cov_file in coverage_files:
            ...
            if cov_data:
                ...
                analysis['uncovered_lines'] += cov_data.get('lines_valid', 0) - cov_data.get('lines_covered', 0)
                analysis['uncovered_branches'] += cov_data.get('branches_valid', 0) - cov_data.get('branches_covered', 0)
                coverage_count += 1
                ...
                for package in cov_data.get('packages', []):
                    module_analysis = {
                        ...
                        'risk_level': self.calculate_risk_level(package['line_rate'], package.get('complexity', 0))
                    }
                    ...
                    if package['line_rate'] < 60.0:
                        analysis['low_coverage_files'].append({
                            ...
                            'priority': 'high' if package['line_rate'] < 40.0 else 'medium'
                        })
    ...
    def analyze_performance_quality(self, artifacts_dir: Path) -> Dict[str, Any]:
        ...
        if execution_times:
            ...
            analysis['performance_trends']['p95'] = statistics.quantiles(execution_times, n=20)[18] if len(execution_times) > 20 else max(execution_times)
            analysis['performance_trends']['p99'] = statistics.quantiles(execution_times, n=100)[98] if len(execution_times) > 100 else max(execution_times)
    ...
    def analyze_security_quality(self, artifacts_dir: Path) -> Dict[str, Any]:
        ...
        for safety_file in safety_files:
            try:
                ...
                for vuln in safety_data:
                    analysis['critical_security_issues'] += 1  # Treat dependency vulnerabilities as critical
                    analysis['total_security_issues'] += 1
                    
                    package_name = vuln.get('package', 'unknown')
                    if package_name not in [pkg['name'] for pkg in analysis['affected_packages']]:
        ...
        if analysis['critical_security_issues'] > 0 or analysis['high_security_issues'] > 5:
    ...
    def generate_quality_metrics(self, artifacts_dir: Path) -> Dict[str, QualityMetric]:
        ...
        metric_definitions = [
            ...
            ('flaky_test_percentage', execution_analysis['flaky_test_percentage'], 'test_execution'),
            ('average_execution_time', execution_analysis['average_execution_time'], 'test_execution'),
            ...
            ('average_response_time', performance_analysis['average_response_time'], 'performance'),
            ('peak_memory_usage', performance_analysis['peak_memory_usage'], 'performance'),
            ('critical_security_issues', security_analysis['critical_security_issues'], 'security'),
            ('high_security_issues', security_analysis['high_security_issues'], 'security'),
        ...
        for metric_name, current_value, category in metric_definitions:
            ...
            if previous_value is not None:
                ...
                if abs(change_percentage) <= self.config['regression_threshold']:
                    ...
                    if metric_name in ['success_rate', 'line_coverage', 'branch_coverage', 'security_score']:
                        trend = 'improving'
                    else:
                        trend = 'degrading'
                else:
            ...
            for threshold in self.quality_thresholds:
                if threshold.metric == metric_name:
                    if threshold.operator == 'gte' and current_value < threshold.threshold:
                        status = 'fail'
                    elif threshold.operator == 'lte' and current_value > threshold.threshold:
                        status = 'fail'
                    elif threshold.operator == 'eq' and current_value != threshold.threshold:
                        status = 'fail'
                    break
        ...
        for category in category_weights.keys():
            ...
            if category_metrics:
                ...
                if category == 'test_execution':
                    ...
                    success_rate = next((m.current_value for m in category_metrics if 'success_rate' in m.name), 95.0)
                    flaky_percentage = next((m.current_value for m in category_metrics if 'flaky' in m.name), 0.0)
                    ...
                    coverages = [m.current_value for m in category_metrics if 'coverage' in m.name]
                    category_scores[category] = statistics.mean(coverages) if coverages else 0.0
                elif category == 'performance':
                    # Based on execution time (inverted - lower is better)
                    avg_time = next((m.current_value for m in category_metrics if 'response_time' in m.name), 60.0)
                    category_scores[category] = max(0, 100 - (avg_time / 10))  # Scale appropriately
                elif category == 'security':
                    # Use security score directly
                    security_score = next((m.current_value for m in category_metrics if 'security_score' in m.name), 100.0)
    ...
    def assess_quality_risks(self, metrics: Dict[str, QualityMetric]) -> Dict[str, Any]:
        ...
        for metric in metrics.values():
            ...
            if metric.trend == 'degrading' and abs(metric.change_percentage) > self.config['regression_threshold']:
            ...
            if metric.status == 'fail':
                risks['risk_factors'].append({
                    ...
                    'current_value': metric.current_value,
                    'category': metric.category,
                    'severity': 'high' if metric.category == 'security' else 'medium'
                })
                high_risk_count += 1
    ...
    def generate_recommendations(self, metrics: Dict[str, QualityMetric]) -> List[Dict[str, str]]:
        """
...
def main():
    ...
    parser.add_argument('--threshold-file', 
                       help='YAML file with custom quality thresholds')
    parser.add_argument('--output', required=True,
                       help='Output JSON file for quality analysis')
    parser.add_argument('--config',
                       help='Configuration file for analysis parameters')
