...
class EnterpriseTestReporter:
    ...
    def calculate_metrics(self) -> None:
        ...
        if self.coverage_data:
            coverage_percentage = sum(cov.get('line_rate', 0.0) for cov in self.coverage_data) / len(self.coverage_data)
    ...
    def generate_visualizations(self) -> Dict[str, str]:
        ...
        for category_name, category_data in self.test_data.items():
            if category_data:
                total_category_tests = sum(suite.get('tests', 0) for suite in category_data)
        ...
        if categories:
            fig_categories = go.Figure(data=[
                go.Bar(x=categories, y=category_counts, marker_color='#007bff')
            ])
        ...
        if self.coverage_data:
            ...
            for cov_data in self.coverage_data:
                for package in cov_data.get('packages', [])[:10]:  # Top 10 packages
                    package_names.append(package['name'].split('.')[-1])  # Just the last part
            ...
            if package_names:
                fig_coverage = go.Figure(data=[
                    go.Bar(
                        x=package_names,
                        y=coverage_rates,
                        marker_color=['#28a745' if x >= 80 else '#ffc107' if x >= 60 else '#dc3545' for x in coverage_rates]
                    )
                ])
                ...
                visualizations['coverage_breakdown'] = fig_coverage.to_html(
                    include_plotlyjs=False,
                    div_id="coverage-breakdown-chart"
                )
        ...
        if self.security_data['total_security_issues'] > 0:
            ...
            visualizations['security_breakdown'] = fig_security.to_html(
                include_plotlyjs=False,
                div_id="security-breakdown-chart"
            )
    ...
    def check_quality_gates(self) -> Dict[str, Any]:
        ...
        success_gate = {
            ...
            'status': 'PASS' if self.metrics['success_rate'] >= self.quality_thresholds['success_rate'] else 'FAIL'
        }
        ...
        coverage_gate = {
            ...
            'status': 'PASS' if self.metrics['coverage_percentage'] >= self.quality_thresholds['coverage'] else 'FAIL'
        }
        ...
        security_gate = {
            ...
            'status': 'PASS' if self.security_data['total_security_issues'] <= self.quality_thresholds['security_issues'] else 'FAIL'
        }
