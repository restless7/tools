...
class TestReportGenerator:
    ...
    def __init__(self, project_name: str = "Test Suite", 
    ...
    def _build_report(self, data: Dict[str, Any]) -> TestReport:
        ...
        visualization_data = self._prepare_visualization_data(metrics, categories, coverage_details)
    ...
    def _extract_categories(self, data: Dict[str, Any]) -> List[TestCategory]:
        ...
        if categorized_total < total_tests:
            other_category = TestCategory(
                ...
                passed=pytest_data.get("summary", {}).get("passed", 0) - sum(cat.passed for cat in categories),
                failed=pytest_data.get("summary", {}).get("failed", 0) - sum(cat.failed for cat in categories)
            )
    ...
    def _generate_recommendations(self, metrics: TestMetrics, gates: Dict[str, bool]) -> List[str]:
        """Generate recommendations based on test results."""
        recommendations = []
        
        if not gates["all_tests_passing"]:
            recommendations.append(f"🔴 Fix {metrics.failed_tests + metrics.error_tests} failing tests")
        
        if not gates["coverage_threshold"]:
            recommendations.append(f"📊 Increase test coverage from {metrics.coverage_percentage:.1f}% to 78%+")
        
        if not gates["performance_acceptable"]:
            recommendations.append(f"⚡ Optimize test execution time (currently {metrics.execution_time:.1f}s)")
        ...
        if gates["all_tests_passing"] and gates["coverage_threshold"]:
            recommendations.append("✅ Excellent test suite! Consider adding more integration tests")
    ...
    def _analyze_codebase(self) -> CodeAnalytics:
        ...
        try:
            ...
            analytics.test_to_code_ratio = (test_lines / max(total_lines - test_lines, 1)) * 100
            ...
            try:
                ...
                analytics.contributors = len(set(commit.author.email for commit in commits))
            ...
            analytics.complexity_score = min(100.0, analytics.lines_of_code / 100)  # Simplified
            analytics.technical_debt_minutes = max(0, (100 - analytics.test_to_code_ratio) * 2)
    ...
    def _calculate_test_velocity(self, data: Dict[str, Any], metrics: TestMetrics) -> TestVelocity:
        ...
        try:
            ...
            velocity.average_test_duration = metrics.execution_time / max(metrics.total_tests, 1)
            velocity.velocity_score = metrics.total_tests / max(metrics.execution_time, 0.1)
            ...
            velocity.parallelization_efficiency = min(100.0, (ideal_time / max(velocity.total_execution_time, 0.1)) * 100)
    ...
    def _generate_quality_insights(self, metrics: TestMetrics, categories: List[TestCategory]) -> QualityInsights:
        ...
        try:
            ...
            if categories:
                ...
                insights.test_distribution_balance = (min_category / max(max_category, 1)) * 100
            ...
            if metrics.failed_tests > 0:
                insights.predicted_issues.append(f"Test failures may indicate unstable code areas")
            if metrics.coverage_percentage < 70:
                insights.predicted_issues.append(f"Low coverage increases risk of undetected bugs")
            if metrics.execution_time > 120:  # 2 minutes
                insights.predicted_issues.append(f"Long test execution may slow development cycle")
    ...
    def _prepare_visualization_data(self, metrics: TestMetrics, categories: List[TestCategory], coverage: Dict[str, Any]) -> VisualizationData:
        ...
        try:
            ...
            viz_data.category_distribution = {
                cat.name: cat.total for cat in categories
            }
    ...
    def _generate_enhanced_recommendations(self, metrics: TestMetrics, gates: Dict[str, bool], 
                                         insights: QualityInsights, analytics: CodeAnalytics) -> List[str]:
        ...
        if gates["all_tests_passing"] and gates["coverage_threshold"] and insights.risk_assessment == "low":
            recommendations.append("✅ Excellent test suite! Consider adding mutation testing")
            recommendations.append("🎯 Explore property-based testing for edge case discovery")
        ...
        if len(recommendations) == 0:
            recommendations.append("🎉 Test suite is performing excellently across all metrics!")
    ...
    def _build_html_template(self, report: TestReport) -> str:
        ...
        status_class = "success" if report.quality_gates["all_tests_passing"] else "failure"
...
<body>
    ...
    def _generate_categories_html(self, categories: List[TestCategory]) -> str:
        ...
        for category in categories:
            pass_rate = (category.passed / category.total * 100) if category.total > 0 else 0
            html += f'''
    ...
    def _prepare_pdf_data(self, report: TestReport) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "PASSED" if report.quality_gates["all_tests_passing"] else "FAILED"
        }
    ...
    def _generate_executive_summary(self, report: TestReport) -> Optional[Path]:
        ...
        try:
            summary_data = {
                ...
                "quality_summary": {
                    "gates_passed": sum(1 for passed in report.quality_gates.values() if passed),
                    "total_gates": len(report.quality_gates),
                    "critical_issues": report.metrics.failed_tests + report.metrics.error_tests
                },
                ...
                ] if report.metrics.failed_tests > 0 else [
...
def main():
    ...
    parser.add_argument("--pdf-service", default="http://localhost:4000", help="PDF service URL")
    parser.add_argument("--no-run", action="store_true", help="Don't run tests, use existing results")
    
    args = parser.parse_args()
    
    generator = TestReportGenerator(
        project_name=args.project_name,
        pdf_service_url=args.pdf_service
    )
