...
class TestRunner:
    ...
    def run_tests(self, test_path: str = "tests/", markers: List[str] = None, 
                 coverage: bool = True):
        ...
        def execute():
            try:
                ...
                if coverage:
                    cmd.extend(['--cov=ice_pipeline', '--cov-report=html', '--cov-report=json'])
                ...
                test_runs.append({
                    ...
                    'duration': (datetime.now() - datetime.fromisoformat(current_test_run['start_time'])).total_seconds(),
                    'tests': len(current_test_run['tests']),
                    'passed': sum(1 for t in current_test_run['tests'] if t.get('outcome') == 'passed'),
                    'failed': sum(1 for t in current_test_run['tests'] if t.get('outcome') == 'failed'),
                })
...
def get_test_results():
    ...
    return jsonify({
        ...
        'summary': {
            ...
            'passed': sum(1 for t in current_test_run['tests'] if t.get('outcome') == 'passed'),
            'failed': sum(1 for t in current_test_run['tests'] if t.get('outcome') == 'failed'),
            'skipped': sum(1 for t in current_test_run['tests'] if t.get('outcome') == 'skipped'),
        }
    })
...
def get_stats():
    ...
    stats = {
        ...
        'average_pass_rate': sum(r['passed'] / max(r['tests'], 1) for r in recent_runs) * 100 / len(recent_runs),
        'trend': {
